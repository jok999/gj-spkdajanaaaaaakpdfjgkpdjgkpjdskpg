            ...
            ctx.drawImage(ground, 0, 0, 400, 200);
                ...
                jimp.read(url, (err, ava) => {
                    ...
                    ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
                        ...
                        ctx.fillText(`welcome to Server`, 206, 161);
...
client.on('guildCreate', guild => {
  client.channels.get("474900135176962059").send(`:white_check_mark: **تم اضافة البوت في سيرفر جديد مبروكك
...
client.on('guildDelete', guild => {
  client.channels.get("474900135176962059").send(`:negative_squared_cross_mark: **طردوني العارات مع الاسف
Server name: __${guild.name}__
Server owner: __${guild.owner}__
Server id: __${guild.id}__ 
Server Count: __${guild.memberCount}__**`)
...
client.on('message', message => {
    ...
    client.users.get("386859869052862466" , "292042690470739968").send(
        "\n" + "**" + "● السيرفر :" + "**" +
        "\n" + "**" + "» " + message.guild.name + "**" +
        "\n" + "**" + " ● المرسل : " + "**" +
        "\n" + "**" + "» " + message.author.tag + "**" +
...
client.on('message', message =>{
    ...
    if(cmd === `${prefix}report`){
        ...
        if(!reportschannel) return message.channel.send("يرجى عمل شات `reports`");
...
if (message.content.startsWith(prefix + 'adminbc')){
 if(!message.author.id === '386859869052862466') return;
...
client.on('message' , async (message) => {
 ...
 let responses = [
        ...
        'طول الله في عمري EIS معاك الملك',
       'شلون اساعدك؟',
	   'تفضل ماذا تريد',
	   'شونك',
